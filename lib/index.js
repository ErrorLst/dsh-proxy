...
// fetches a health-check URL THROUGH the proxy. After `healthCheckFailures`
// consecutive failures the proxy is declared broken; the browser half polls
// the `/dsh-proxy/status` endpoint (registered on the webServer service) and
...
export function apply(ctx, config = {}) {
  ...
  // One check = first attempt; on failure, wait `retryDelayMs` and try once
  // more. Only when BOTH attempts fail does this check count as a failure —
  // a successful retry resets the counter like a successful first attempt.
  const runCheck = async () => {
    if (checking) return;
    checking = true;
    try {
      const first = await attemptCheck();
      if (first.ok) {
        health.failures = 0;
        health.lastError = null;
      } else {
        await new Promise((resolve) => setTimeout(resolve, health.retryDelayMs));
        const retry = await attemptCheck();
        if (retry.ok) {
          health.failures = 0;
          health.lastError = null;
        } else {
          health.failures += 1;
          health.lastError = retry.error;
        }
      }
    } finally {
  ...
  const schedule = (delayMs) => {
    checkTimer = setTimeout(async () => {
      const value = source() ?? {};
      await runCheck();
      schedule((value.healthCheckInterval ?? DEFAULT_CHECK_INTERVAL) * 1000);
    }, delayMs);
  };
