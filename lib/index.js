...
const DEFAULT_CHECK_INTERVAL = 30; // seconds between checks
const DEFAULT_CHECK_THRESHOLD = 3; // consecutive failures before "broken"
const CHECK_TIMEOUT_MS = 10000; // per-check timeout
  ...
  const markBrokenOrOk = () => {
    if (health.failures >= health.threshold) {
      if (health.state !== "broken") {
        health.state = "broken";
        ctx.logger.warn(
          `[dsh-proxy] 代理疑似失效：连续 ${health.failures} 次访问 ${health.checkUrl} 失败（${health.lastError}）`,
        );
      }
    } else {
      if (health.state === "broken") {
